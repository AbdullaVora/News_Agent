...
import { Sparkles, Send, Newspaper, TrendingUp, Earth, Coins, Bitcoin, Bot } from 'lucide-react';
...
//     { label: 'World Top News', icon: Earth },
...
export function ChatInterface({ onSendMessage, messages, onTitleClick }: {
  onSendMessage: (message: string) => void;
  messages: Message[];
  onTitleClick?: (id: string) => void;
}) {
  ...
  const quickActions = [
    { label: 'Latest Tech News', icon: Newspaper },
    { label: 'World Top News', icon: Earth },
    { label: 'Latest AI News', icon: Bot },
    { label: 'Crypto trends', icon: Bitcoin  }
  ];
  ...
  return (
    <div className="h-full flex flex-col bg-transparent">
      ...
      {/* Quick Actions - ALWAYS SHOWN (removed condition) */}
      <div className="px-6 pb-4">
      ...
      {/* Input Area */}
      <div className="p-6 border-t border-gray-700 bg-card/20 backdrop-blur-sm">
        <div className="flex gap-2" onSubmit={handleSubmit}>
          <Input
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            onKeyDown={(e) => {
